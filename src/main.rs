...
fn main() {
    ...
    let _mesh = cube.triangulation(0.01).to_polygon();
    ...
    let _mesh = cylinder.triangulation(0.01).to_polygon();
    ...
    let _mesh = and_result.unwrap().triangulation(0.01).to_polygon();
    ...
    let _mesh = or_result.unwrap().triangulation(0.01).to_polygon();
    //let file = std::fs::File::create("test_OR.obj").unwrap();
    //obj::write(&mesh, file).unwrap();
    // This results in a cube on a stick, aka the union of the cube and the cylinder
    // Aka the region of space which is inside the cube OR inside the cylinder
    ...
    let _mesh = and_not_result.unwrap().triangulation(0.01).to_polygon();
